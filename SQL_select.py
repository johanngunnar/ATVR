...
for x in arr:
    print("   ", x)
    if x[17] == '420369-7789':
    	alag_olgerdin = alag_olgerdin + (x[20]*(x[5])
    alag = (alag) + (x[20]*x[5])

    for i in slot_dict:
        if x[17] == i:
            alag_arr[slot_dict[i]] = alag_arr[slot_dict[i]] + (x[20]*x[5])
...
print('{}:{} '.format( math.floor(start_min_from_midnight/time_slot),start_min_from_midnight%time_slot))  
