...
var EIM{t in 1..T, d in 1..D} binary;
var SAM{t in 1..T, d in 1..D} binary;
/*------------------------*/
...
/* ------ Markfall & Skorður --------*/
minimize MaxAlag: 5*TheMaxAlag + sum{t in 1..T, d in 1..D} SuperAlag[t,d] + TheMaxAlagOlgerdin + sum{t in 1..T, d in 1..D}(LB1[t,d]+ LB2[t,d]+ LB3[t,d]+ LB4[t,d]+ LB5[t,d]+ LB6[t,d]+ LB7[t,d]);

s.t. Alagsmaeling{t in 1..(T-windowsize), d in 1..D}: sum{s in 1..S, k in t..(t+windowsize)} A[s]*x[s,k,d] <= (Ttarget[t,d] + SuperAlag[t,d]);

s.t. MaxAlagMaeling{t in 2..T, d in 1..D}: TheMaxAlag >= SuperAlag[t,d];
s.t. MaxAlagMaelingOlgerdin{t in 1..1, d in 1..D}: TheMaxAlagOlgerdin >= SuperAlag[t,d];
...
s.t. MaxOnePerDayEIM{d in 1..D}: sum{t in 1..T} EIM[t,d] <= 1;
s.t. MaxOnePerDaySAM{d in 1..D}: sum{t in 1..T} SAM[t,d] <= 1;
...
s.t. MaxNrOfTimeSlotsEIM: sum{t in 1..T, d in 1..D} EIM[t,d] <= 5;
s.t. MaxNrOfTimeSlotsSAM: sum{t in 1..T, d in 1..D} SAM[t,d] <= 5;
...
s.t. GroupSendingarEIM{s in Eimskip, t in 1..T,d in 1..D}: x[s,t,d] <= EIM[t,d];
s.t. GroupSendingarSAM{s in Samskip, t in 1..T,d in 1..D}: x[s,t,d] <= SAM[t,d];
/*--------------------------------*/
